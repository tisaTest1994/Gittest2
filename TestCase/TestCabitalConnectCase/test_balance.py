    ...
    def test_balance_001(self, partner):
        ...
        with allure.step("验签"):
            ...
            sign = ApiFunction.make_signature(unix_time=str(unix_time), method='GET',
                                              url='/api/v1/accounts/{}/balances'.format(account_vid),
        ...
        with allure.step("账户可用余额列表"):
            r = session.request('GET', url='{}/accounts/{}/balances'.format(connect_url, account_vid),
        ...
        with allure.step("判断提供给partner的和我们自用的值一致"):
            for i in get_json(file='partner_info.json')[get_json()['env']][partner]['config_info']['currencies']:
                with allure.step("识别usdc"):
                    if i['symbol'] == 'USDC':
                        i['symbol'] = 'USD'
                mobile_balance = ApiFunction.get_crypto_number(type=i['symbol'], balance_type='BALANCE_TYPE_AVAILABLE',
                                                               wallet_type='BALANCE')
                for y in r.json()['balances']:
                    if y['code'] == i['symbol']:
                        assert float(mobile_balance) == float(
                            y['balances']), '币种{}判断提供给{}的和我们自用的值一致失败，我们自用balance是{},bybit是{}'.format(i['symbol'],
                                                                                                     partner,
                                                                                                     mobile_balance,
                                                                                                     y['balances'])
            ...
            for i in get_json(file='partner_info.json')[get_json()['env']][partner]['config_info']['currencies']:
                ...
                with allure.step("账户可用余额列表"):
                    r = session.request('GET',
                                        url='{}/accounts/{}/balances/{}'.format(connect_url, account_vid, i['symbol']),
                                        headers=connect_headers)
                with allure.step("状态码和返回值"):
                    logger.info('状态码是{}'.format(str(r.status_code)))
                    logger.info('返回值是{}'.format(str(r.text)))
                ...
                with allure.step("通过mobile接口获取金额"):
                    mobile_balance = ApiFunction.get_crypto_number(type=i['symbol'],
                ...
                with allure.step("获取的金额和通过mobile接口获取的金额对比"):
                    assert float(mobile_balance) == float(r.json()['balance'][
                                                              'balances']), '币种{}判断提供给infinni games的和我们自用的值一致失败，我们自用balance是{},bybit是{}'.format(
