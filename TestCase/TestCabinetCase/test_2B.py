...
class TestOperateApi:
    ...
    @allure.title('test_2B_001 提现USDC成功')
    ...
    @allure.title('test_2B_002 申购活期产品成功')
    ...
    @allure.title('test_2B_003 申购定期产品成功')
    ...
    @allure.title('test_2B_004 赎回活期产品成功')
    ...
    @allure.title('test_2B_005 定期产品打开auto renew')
    def test_2B_005(self):
        ...
        with allure.step("校验状态码"):
            assert r.status_code == 200, "http 状态码不对，目前状态码是{}".format(r.status_code)
        with allure.step("校验返回值"):
            assert 'update_result' in r.text, "查询update_result错误，返回值是{}".format(r.text)

    @allure.title('test_2B_006 定期产品关闭auto renew')
    def test_2B_006(self):
        with allure.step("定期产品关闭auto renew"):
            data = {
                "auto_renew": False
            }
            r = session.request('POST', url='{}/operatorapi/account/{}/renew/{}'.format(operateUrl,
