...
class TestAccountApi:
    ...
    def test_information_match_fail_001(self):
        with allure.step("测试账号"):
            account_id = 'bacf2b3e-6599-44f4-adf6-c4c13ff40946'  # 此处有疑问？？？重复的account提交的时候 status_code就400了
        with allure.step("name match 数据"):
            data = {
                'name': 'qq',
                'id': '1122',
    ...
    def test_information_match_fail_002(self):
        with allure.step("准备参数"):
            account_id = 'fd44d09d-14e2-4775-9532-b93aed97acf6'
        with allure.step("name match 数据"):
            data = {
                'name': '',
                'id': '1122',
    ...
    def test_information_match_fail_006(self):
        ...
        with allure.step("校验返回值"):
            logger.info('match接口返回值是{}'.format(str(r.text)))
            logger.info("match_status==>> 期望结果:PA025	用户生日不合法,实际结果:【{}】".format(r.json()['code']))
            assert r.json()['code'] == ' PA025 '
