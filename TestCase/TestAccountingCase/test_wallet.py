        ...
        for i in range(1, OperateExcel.get_excel_sheet_all_row_number('Cash Wallet')):
            ...
            with allure.step("借贷关系"):
                if 'Debit' in str(line_info[13]).split(':')[1]:
                    ...
                    balance_direction = 1
            sql = "select * from wallet where wallet_name = {} and code = {} and account_code = {} and status = {} and account_type = {} and allow_overdraft = {} and balance_direction = {};".format(str(line_info[3]).split(':')[1], str(line_info[5]).split(':')[1], str(line_info[10]).split(':')[1], status, account_type, allow_overdraft, balance_direction)
