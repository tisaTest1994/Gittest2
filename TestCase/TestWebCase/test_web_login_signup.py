...
class TestWebLogin:
    ...
    def test_signup_001(self):
        ...
        with allure.step("邮箱含中文，如测试@163.com"):
            ...
            assert operate_element_web(self.driver, 'signupPage', "click_sendcode", 'check_enabled'), "send code显示状态错误"
    ...
    def test_signup_002(self):
        ...
        with allure.step("点击send code,验证码输入框字符限制6位验证"):
            ...
            operate_element_web(self.driver, 'signupPage', "mailcode", "input", "6666666")
            # 输入7个6，看value是否为6个6。
    ...
    def test_signup_003(self):
        ...
        with allure.step("设置密码-正确密码格式,并密码显隐切换"):
            operate_element_web(self.driver, 'signupPage', "signup_form_password_input", "input", "Aaaa1234")
            assert self.driver.find_element_by_xpath('//*[@id="signupForm"]/div[4]/p'), "输入正确格式密码，提示信息未消失"
            # Password显隐默认不显示
            assert operate_element_web(self.driver, 'signupPage', "VisibilityOffIcon", "check"),\
                "密码显隐默认状态错误"
            # 密码默认不显示，点击密码显隐按钮后密码显示
            operate_element_web(self.driver, 'signupPage', "VisibilityOffIcon")
            assert self.driver.find_element_by_xpath('//*[@id="signup_form_password_input"][@type="text"]'),\
                "密码未显示"
            # 再次点击密码显隐按钮，密码隐藏
            operate_element_web(self.driver, 'signupPage', "VisibilityIcon")
            assert self.driver.find_element_by_xpath('//*[@id="signup_form_password_input"][@type="password"]'),\
                "密码未隐藏"
        with allure.step("错误密码格式1：密码设置位数少于8位"):
            operate_element_web(self.driver, 'signupPage', "VisibilityOffIcon")
            operate_element_web(self.driver, 'signupPage', "signup_form_password_input", "delete")
            operate_element_web(self.driver, 'signupPage', "signup_form_password_input", "input", "Aaaa123")
        ...
        with allure.step("错误密码格式2：含有非法字符"):
            operate_element_web(self.driver, 'signupPage', "signup_form_password_input", "delete")
            operate_element_web(self.driver, 'signupPage', "signup_form_password_input", "input", "你好中国!@#1")
    ...
    def test_signup_004(self):
        ...
        with allure.step("点击协议同意书前复选框"):
            # 复选框默认不勾选,点击复选框
            assert (operate_element_web(self.driver, 'signupPage', "signup_termschk", "check_selected")) is False,\
                "复选框默认状态错误"
            operate_element_web(self.driver, 'signupPage', "signup_termschk")
            assert operate_element_web(self.driver, 'signupPage', "signup_termschk", "check_selected"), "复选框勾选失败"
        # with allure.step("点击Terms of Services,然后并关闭页面"):
        #     operate_element_web(self.driver, 'signupPage', "service")
        #     assert operate_element_web(self.driver, "signupPage", "full-width-tab-0", "check"), "Terms of Services页面未显示"
        #     # 关闭窗口的close按钮缺一个id
        #     # operate_element(self.driver, 'signupPage', "")
        ...
        #     operate_element_web(self.driver, 'signupPage', "privacy")
        #     assert operate_element_web(self.driver, "signupPage", "full-width-tab-0", "check"), "Privacy Policy页面未显示"

    @allure.title('test_web_signup_005')
    @allure.description('注册账号')
    def test_signup_005(self):
        webFunction.signup_web(self)


    @allure.title('test_web_signup_006')
    @allure.description('signup页面，login按钮跳转')
    def test_signup_006(self):
        operate_element_web(self.driver, 'loginPage', "login_gotosignup")
        operate_element_web(self.driver, 'signupPage', "signup_gotologin")
        assert operate_element_web(self.driver, 'loginPage', "login_gotosignup", "check"), "log in按钮跳转失败"
    ...
    def test_login_002(self):
        ...
        with allure.step("输入账号"):
            operate_element_web(self.driver, 'loginPage', 'username', 'delete')
            operate_element_web(self.driver, 'loginPage', 'username', 'input', input_string=account)
        with allure.step("输入密码"):
            operate_element_web(self.driver, 'loginPage', 'sign_password_input', 'delete')
            operate_element_web(self.driver, 'loginPage', 'sign_password_input', 'input', input_string=password)
        with allure.step("点击sign in"):
            operate_element_web(self.driver, 'loginPage', 'login_login')
            sleep(2)
        with allure.step("确定已经登录成功到首页"):
            assert operate_element_web(self.driver, 'assetPage', 'Cabital Logo', 'check'), '未成功登录'
            sleep(2)
    ...
    def test_login_003(self):
        with allure.step("确认弹出重置密码框"):
            operate_element_web(self.driver, 'loginPage', 'login_forgotpw', 'check'), '未弹出重置密码框'
