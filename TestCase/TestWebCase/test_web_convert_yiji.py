...
class TestWebConvert:
    ...
    def test_convert_001(self, chrome_driver):
        ...
        for i in range(0, len_account):
            ...
            with allure.step("点击convert按钮"):
                operate_element_web(chrome_driver, 'convertPage', 'mainpage_menu_buysell')
                if i == 0:
                    with allure.step("检查页面提示信息"):
                        assert operate_element_web(chrome_driver, '', 'Verify your identity before convert',
                                                   'check'), "当前账号的kyc状态是:{}, 未弹出Verify your identity before convert提示框".format(
                            account_list[test_account])
                    with allure.step("点击Verify your identity before convert后 跳转至Identity Verification认证页面"):
                        operate_element_web(chrome_driver, '', 'Verify your identity before convert')
                    ...
                    with allure.step("点击Verify your identity before convert后，跳转至Identity Verification认证页面"):
                        operate_element_web(chrome_driver, '', 'Verify your identity before convert')
                        assert operate_element_web(chrome_driver, '', 'Identity Verification',
                                                   'check'), "未跳转至Identity Verification页面"
                elif i == 3:
                    with allure.step("检查弹出提示框"):
                        assert operate_element_web(chrome_driver, '', 'Verify your identity before convert',
                                                   'check'), "当前账号的kyc状态是:{}, 未弹出Verify your identity before convert提示框".format(
                            account_list[test_account])
                    with allure.step("点击Verify your identity before convert后，跳转至account页面"):
                        operate_element_web(chrome_driver, '', 'Verify your identity before convert')
                        assert operate_element_web(chrome_driver, '', 'Account Information', 'check'), '未返回account页面'
                elif i == 4:
                    with allure.step("检查是否打开convert页面"):
            ...
            for i in ApiFunction.get_cfx_list():
                with allure.step("正向币种对，major_ccy 是buy值"):
                    ...
                    with allure.step("随机生成要输入的金额"):
                        ...
                        operate_element_web(chrome_driver, 'convertPage', 'convert_buy_input', "input",
                                            transaction['data']['buy_amount'])
                    ...
                    with allure.step("检查my assets"):
                        operate_element_web(chrome_driver, 'convertPage', 'convert_check_myasset')
                with allure.step("正向币种对，major_ccy 是sell值"):
                    ...
                    with allure.step("随机生成要输入的金额"):
                        ...
                        operate_element_web(chrome_driver, 'convertPage', 'convert_sell_input', "input",
                                            transaction['data']['sell_amount'])
                    with allure.step("点击convert按钮"):
                        operate_element_web(chrome_driver, 'convertPage', 'dialog_convert_convert')
                        sleep(1)
                    with allure.step("获得换汇后buy币种balance金额"):
    ...
    def test_convert_004(self, chrome_driver):
        ...
        with allure.step("检查默认sell币种的available balance金额显示"):
            ...
            with allure.step("判断页面金额显示是否和接口返回数据一致"):
                available_balance = chrome_driver.find_element_by_xpath(
                    "//*[@id='convert_sell_input']/../../../div").text
    ...
    def test_convert_005(self, chrome_driver):
        ...
        with allure.step("点击max按钮"):
            ...
            assert operate_element_web(chrome_driver, 'convertPage', 'convert_sell_input',
                                       'get_value') == sell_amount_wallet_balance, 'max按钮未自动填入最大可用金额'
