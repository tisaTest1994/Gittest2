...
class TestWebTransactionApi:
    ...
    def setup_method(self):
        ...
        operate_element_web(self.driver, 'assetPage', 'header-desktop-menu-item-CB284')
        ...
        for i in ApiFunction.balance_list():
            ...
            with allure.step("通过页面获得某个币种的交易记录"):
                with allure.step("清除搜索记录"):
                    operate_element_web(self.driver, 'transactionPage', 'transaction-filter-currency', type='delete')
                with allure.step("搜索特定币种"):
                    operate_element_web(self.driver, 'transactionPage', 'transaction-filter-currency', type='input', input_string=i)
                    sleep(1)
                with allure.step("点击指定币种"):
                    operate_element_web(self.driver, 'transactionPage', 'undefined-option-item-{}'.format(i))
                sleep(10)
