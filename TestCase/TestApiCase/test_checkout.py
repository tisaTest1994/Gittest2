...
class TestCheckoutApi:
    ...
    @allure.title('test_check_out_022')
    @allure.description('EE用户')
    def test_check_out_022(self):
        with allure.step("币种兑选择"):
            pairs = ApiFunction.get_buy_crypto_currency(type='all')
            pairs.remove('USDT-IDR')
        ...
        for z in pairs:
            for x in get_json()['checkOutAreaList2']:
                    ...
                    while 1 < 2:
                        ...
                        data = {
                            ...
                            "destination": {
                                ...
                                "fee": {
                                    "code": (str(crypto_list['pairs']).split('-'))[1],
                                    "amount": str((Decimal(t_fee) * Decimal(crypto_list['quote'])).quantize(Decimal('0.00'), ROUND_CEILING))
                                }
                            }
                        }
                        logger.info('checkout传入参数是{}'.format(data))
                ...
                #                 "fee": {
                #                     "code": (str(crypto_list['pairs']).split('-'))[1],
                #                     "amount": str(float(t_fee) * float(str(crypto_list['quote'])))
                #                 }
                #             }
                #         }
                #         logger.info('checkout传入参数是{}'.format(data))
