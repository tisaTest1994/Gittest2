...
class TestMonitorApi:
    ...
    def test_monitor_001(self):
        ...
        with allure.step("查询创建的cases"):
            ...
            r = session.request('GET', url='{}/operator/cases/{}'.format(monitorUrl, r.json()['caseSystemId']), params=params, headers=headers)
            with allure.step("状态码和返回值"):
                logger.info('状态码是{}'.format(str(r.status_code)))
                logger.info('返回值是{}'.format(str(r.text)))
            with allure.step("校验状态码"):
        ...
        with allure.step("查询创建的cases的audit log"):
            ...
            r = session.request('GET', url='{}/operator/cases/{}/audit'.format(monitorUrl, caseSystemId), params=params, headers=headers)
            with allure.step("状态码和返回值"):
                logger.info('状态码是{}'.format(str(r.status_code)))
                logger.info('返回值是{}'.format(str(r.text)))
            with allure.step("校验状态码"):
    ...
    def test_monitor_002(self):
        ...
        with allure.step("开启ogs"):
            ...
            r = session.request('POST', url='{}/operator/cases/{}/ogs'.format(monitorUrl, caseSystemId), params=json.dumps(data), headers=headers)
            with allure.step("状态码和返回值"):
                logger.info('状态码是{}'.format(str(r.status_code)))
                logger.info('返回值是{}'.format(str(r.text)))
            with allure.step("校验状态码"):
        ...
        with allure.step("关闭ogs"):
            ...
            r = session.request('DELETE', url='{}/operator/cases/{}/ogs'.format(monitorUrl, caseSystemId), params=json.dumps(data), headers=headers)
            with allure.step("状态码和返回值"):
                logger.info('状态码是{}'.format(str(r.status_code)))
                logger.info('返回值是{}'.format(str(r.text)))
            with allure.step("校验状态码"):
    ...
    def test_monitor_003(self):
        ...
        with allure.step("给出建议"):
            ...
            r = session.request('POST', url='{}/operator/cases/{}/suggestion'.format(monitorUrl, r.json()['caseSystemId']), data=json.dumps(data), headers=headers)
            with allure.step("状态码和返回值"):
                logger.info('状态码是{}'.format(str(r.status_code)))
                logger.info('返回值是{}'.format(str(r.text)))
            with allure.step("校验状态码"):
                assert r.status_code == 200, "http状态码不对，目前状态码是{}".format(r.status_code)
            with allure.step("校验返回值"):
                assert r.json()['externalCaseId'] is not None, '给出建议错误,返回值是{}'.format(r.text)
