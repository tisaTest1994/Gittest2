...
class TestCoreApi:
            ...
            for i in get_json()['show_list']:
                ...
                with allure.step("cumulative_interest计算"):
                    ...
                    if Decimal(r.json()['cumulative_interest']) != Decimal(sum(all_interest)):
                        assert Decimal(sum(all_interest)) - Decimal(r.json()['cumulative_interest']) >= Decimal(0.5) or Decimal(sum(all_interest)) - Decimal(r.json()['cumulative_interest']) <= Decimal(0.5), '获取所有Saving产品的持有金额详情的已派发利息, 显示币种是{}, 获取累计活期利息是{}, 获取累计定期利息{}, 接口返回的累计利息总和是{}'.format(i, sum(flexible_all_interest_list), sum(fixed_all_interest_list), r.json()['cumulative_interest'])
    ...
    def test_core_018(self):
        headers['Authorization'] = "Bearer " + ApiFunction.get_account_token(account='alice.wang+030705@cabital.com')
        with allure.step("显示币种矩阵"):
            for i in get_json()['show_list']:
                headers['X-Currency'] = i
