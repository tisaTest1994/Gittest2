...
class TestCoreApi:
    ...
    @allure.description('获取所有Saving产品的持有金额详情total_saving_amount计算')
    def test_core_015(self):
        headers['Authorization'] = "Bearer " + ApiFunction.get_account_token(account='external.qa@cabital.com')
        with allure.step("显示币种矩阵"):
            for i in get_json()['show_list']:
                headers['X-Currency'] = i
                with allure.step("获取所有Saving产品的持有金额详情total_saving_amount计算"):
    ...
    @allure.description('获取所有Saving产品的持有金额详情cumulative_interest计算')
    def test_core_016(self):
        headers['Authorization'] = "Bearer " + ApiFunction.get_account_token(account='external.qa@cabital.com')
        with allure.step("显示币种矩阵"):
            for i in get_json()['show_list']:
                headers['X-Currency'] = i
                with allure.step("获取所有Saving产品的持有金额详情cumulative_interest计算"):
                    r = session.request('GET', url='{}/earn/saving/holding/details'.format(env_url), headers=headers)
                with allure.step("状态码和返回值"):
                    logger.info('状态码是{}'.format(str(r.status_code)))
                    logger.info('返回值是{}'.format(str(r.text)))
                ...
                with allure.step("cumulative_interest计算"):
                    ...
                    if Decimal(r.json()['cumulative_interest']) != Decimal(sum(flexible_all_interest_list) + sum(fled_all_interest_list)):
                        if len(str(r.json()['cumulative_interest'])) < len(str(sum(flexible_all_interest_list) + sum(fled_all_interest_list))):
                            assert str(r.json()['cumulative_interest']) == str(sum(flexible_all_interest_list) + sum(fled_all_interest_list))[:-1], '获取所有Saving产品的持有金额详情cumulative_interest计算错误，显示货币类型是{}，返回值是{}".format(i, r.text)'
                        else:
            ...
            for i in get_json()['show_list']:
                ...
                with allure.step("interest_to_settle计算"):
                    ...
                    if Decimal(r.json()['interest_to_settle']) != Decimal(sum(fled_all_interest_list)):
                        if len(str(sum(fled_all_interest_list))) < len(str(sum(fled_all_interest_list))):
                            assert str(r.json()['interest_to_settle']) == str(sum(fled_all_interest_list))[:-1], '获取所有Saving产品的持有金额详情interest_to_settle计算错误，显示货币类型是{}，返回值是{}".format(i, r.text)'
                        else:
            ...
            for i in get_json()['show_list']:
                ...
                with allure.step("cumulative_interest计算"):
                    ...
                    if Decimal(r.json()['total_earnings']) != Decimal(sum(flexible_all_interest_list) + sum(fled_all_interest_list)):
                        if len(str(r.json()['total_earnings'])) < len(str(sum(flexible_all_interest_list) + sum(fled_all_interest_list))):
                            assert str(r.json()['total_earnings']) == str(sum(flexible_all_interest_list) + sum(fled_all_interest_list))[:-1], '获取所有Saving产品的收益cumulative_interest计算错误，显示货币类型是{}，返回值是{}".format(i, r.text)'
                        else:
        ...
        with allure.step("显示币种矩阵"):
            for i in get_json()['show_list']:
                ...
                with allure.step("interest_to_settle计算"):
                    ...
                    logger.info('获取累计定期利息{}'.format(str(sum(fled_all_interest_list))))
                    if Decimal(r.json()['interest_to_settle']) != Decimal(sum(fled_all_interest_list)):
                        if str(sum(fled_all_interest_list)) < str(sum(fled_all_interest_list)):
                            assert str(r.json()['interest_to_settle']) == str(sum(fled_all_interest_list))[:-1], '获取所有Saving产品的收益详情interest_to_settle计算错误，显示货币类型是{}，返回值是{}".format(i, r.text)'
                        else:
                    ...
                    with allure.step("获取累计定期利息"):
                        ...
                        for x in get_json()['crypto_list']:
                            ...
                            fled_all_interest_list.append(
                                Decimal(crypto_len(Decimal(quote['middle']) * sum(fled_all_interest_amounts_list), i)))
                    logger.info('获取累计定期利息{}'.format(str(sum(fled_all_interest_list))))
                    if Decimal(r.json()['fixed_earnings']) != Decimal(sum(fled_all_interest_list)):
                        if str(sum(fled_all_interest_list))[-1] < str(sum(fled_all_interest_list)):
                            assert str(r.json()['fixed_earnings']) == str(sum(fled_all_interest_list))[:-1], '获取所有Saving产品的收益cumulative_interest计算错误，显示货币类型是{}，返回值是{}".format(i, r.text)'
                        else:
                            assert str(r.json()['fixed_earnings'])[:-1] == str(sum(fled_all_interest_list))[:-1], '获取所有Saving产品的收益cumulative_interest计算错误，显示货币类型是{}，返回值是{}".format(i, r.text)'
