...
class OperateExcel:
    ...
    @staticmethod
    def get_product_limit(sheet_name, path=os.path.split(os.path.realpath(__file__))[0] + '/../Resource/Product Limit.xlsx'):
        ...
        for i in range(1, row):
            ...
            product_limit_list.append({'code': str(line[0]).split(':')[1], 'mix': str(line[3]).split(':')[1], 'max': str(line[4]).split(':')[1]})
